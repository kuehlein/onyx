...
4. **No gamified chrome.** No points/XP, badges, levels, leaderboards, **streaks**, streak-freeze/loss-aversion, confetti, trophies, or social mechanics (product-direction.md §9; ux-vision L16/L31/L217). Competence is signaled by honest completion + honest readiness delta. **"Done" = the absence of the Filled button, not a reward animation.**
...
- **`showOnyxSheet<T>` ✅ (built)** — one wrapper over `showModalBottomSheet` in `sheet_header.dart`; the themed sheet radius + drag handle come from `bottomSheetTheme`, `isScrollControlled`/`showDragHandle` default to the common case, `T?` returns are typed, and **`barrierColor` (black @ ~0.45, §4.9)** is applied from here. **All 19 sheet call sites** migrated. **Surface unified:** every sheet now takes the theme's `surfaceContainerHighest` (§2.5 tier 3) — the ~12 `surfaceContainerLow` overrides were removed, and scroll-fades derive from `sheetSurface(context)` (= `bottomSheetTheme.backgroundColor`) so they always match; retuning the sheet surface is now the single `bottomSheetTheme` line. `useSafeArea` is exposed but **off by default** — ~7 sheets hand-size to a fixed screen-height fraction, so global safe-area would clip them on notched devices until they're converted to `maxHeight`. **`showApiKeySheet` ✅ (§4.9)** composes it. Nested detail = in-place `AnimatedSize` sub-section, never a sheet-in-a-sheet.
...
- **Step 5 — Opportunistic token migration (boy-scout rule, ongoing).** Whenever you touch a widget, swap *spacing/radius* literals for `context.tokens.*`. (The color half is done — the `status_colors` shim is retired, Step 7.) Migration dashboard = these shrinking greps:
  ...
  ```
- **Step 6 — Catalog, built *when a screen consumes it* (not preemptively).** Done so far: **`EmptyState` ✅** (glyph→title→message→action, replacing the ad-hoc `Center>Column` blocks; golden + test), **`LoadingView` ✅** (the progress-policy row), **`DestructiveRow` ✅** (settings-#67; golden + test), **`showOnyxSheet` ✅** (all 19 sheet sites). The rest — `Sparkline`/`CoverageBar`/`CompletionRing`/`ForecastBand`/`GoalLaneRow`/`SharedBudgetBar`/`TodayFlowRow`/`InsightTile`/`MixSlider`/`ReadinessBand`/`SkeletonBlock`/`ThinEvidence` — have **no consumer yet** (the only bespoke painter today is `today_ring.dart`). Building them now is speculative; build each as its owning screen is (re)built (the dataviz primitives with the Analytics/Home redo) and **add a golden** at that point (the real restyle regression net). `[reframed 2026-09-17]`
