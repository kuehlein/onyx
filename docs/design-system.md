...
- **`EmptyState`** — muted icon (no hue) · one line · one action. Never a scold. Used by hub-0-goals, no-cards, no-data tiles.
- **`SkeletonBlock` / `SkeletonLaneRow`** — shimmer `surfaceContainerHigh → Highest` over `motionSlow`, `radiusChip`. ×2 skeleton lanes on hub. No spinner-on-empty.
...
- **Step 4 — `code_block.dart` surface + palette.** Move panel to `surfaceContainerLow`, swap in `onyx_code_theme` (F1/F7); drop the `#1D1F21`/`#C5C8C6` fallbacks.
- **Step 5 — Opportunistic token migration (boy-scout rule, ongoing).** Whenever you touch a widget, swap literals for `context.tokens.*`. Migration dashboard = three shrinking greps:
  ```
