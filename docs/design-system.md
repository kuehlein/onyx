...
- **`showOnyxSheet<T>` ✅ (built)** — one wrapper over `showModalBottomSheet` in `sheet_header.dart`; the themed sheet radius + drag handle come from `bottomSheetTheme`, `isScrollControlled`/`showDragHandle` default to the common case, `T?` returns are typed. **All 19 sheet call sites migrated** (was ~15). *Still owed (§4.9):* `barrierColor black@0.45` + `useSafeArea` — deferred to keep the migration zero-visual-change, and now a **one-edit** switch-on in the wrapper. Also open: sheet backgrounds are split (`surfaceContainerHighest` theme default vs. ~12 sheets overriding to `surfaceContainerLow` for fade-edge blending) — an aesthetic-pass call to unify. **`showApiKeySheet` ✅ (built, §4.9)** composes it. Nested detail = in-place `AnimatedSize` sub-section, never a sheet-in-a-sheet.
