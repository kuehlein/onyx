...
- **RS256→HS256 [algorithm confusion](_meta/glossary.md#algorithm-confusion) (key confusion).** If your code calls a generic `verify(token, key)` that picks the algorithm from the header, an attacker switches `alg` to `HS256` and signs with your *public* RSA key as the HMAC secret. Since the public key is, well, public, the forgery verifies. **Fix:** pin the exact expected algorithm per key; keep symmetric and asymmetric keys in separate code paths.
...
- **OAuth 2.1 note** — folds in current best practice: the **implicit** and **resource-owner-password (ROPC)** grants are removed, **[PKCE](_meta/glossary.md#pkce)** is mandatory for all clients, and refresh tokens must rotate or be sender-constrained.
