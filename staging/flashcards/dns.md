...
- Interviewer asks how a URL becomes a TCP connection — DNS is step one
...
| TTL range used in practice | 30 s (fast failover) – 86 400 s (stable CDN assets) |
...
3. Root nameserver → TLD nameserver (.com, .io)
...
- `SOA` — Start of Authority; TTL defaults and zone serial for AXFR
...
- DNS LB is cheaper and globally distributed but cannot detect mid-connection failures; a load balancer can drain connections gracefully
- DNS LB has no session awareness; ELB/ALB can inspect HTTP headers for routing logic
...
- Anycast routes to the topologically nearest instance at the BGP level — more accurate but requires owning an AS and IP block
...
- Sidecars (Envoy/Istio) intercept DNS and add mTLS, retries, and circuit breaking on top

*CDN origin shielding via CNAME chain:*
```
...
- CNAME at apex is illegal per RFC 1034 — use ALIAS/ANAME records (Route 53) or CNAME flattening (Cloudflare) instead
...
- **Relying on source IP for GeoDNS:** The resolver IP (not client IP) is used for geo-lookup. EDNS Client Subnet (ECS) partially addresses this but is opt-in.
- **DNS amplification in DDoS:** Open resolvers can be used to amplify UDP traffic. Authoritative servers should restrict recursion; use rate limiting and anycast to absorb volumetric attacks.
...
- *Designing Data-Intensive Applications* — Chapter 1 (reliability/scalability framing applicable to DNS SLAs)
