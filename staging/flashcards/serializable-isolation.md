...
- **Predicate / index-range locks are what make 2PL prevent phantoms.** Locking existing rows isn't enough when the anomaly is about rows that *don't exist yet*; you must lock the *predicate* (all rows matching a `WHERE`, including future inserts). In practice engines approximate predicate locks with **index-range locks** (e.g. InnoDB **next-key locks** = row lock + gap lock).
