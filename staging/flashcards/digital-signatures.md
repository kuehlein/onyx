...
- One party authorizes; **many independent parties** must verify the same artifact (signed firmware, [TLS](_meta/glossary.md#tls) certificate chains, [JWT](_meta/glossary.md#jwt)s with `RS256`/`ES256`).
...
**Do not use when:**
- Both endpoints already **share a symmetric key** and non-repudiation is not needed -> use **HMAC** (far faster, no [PKI](_meta/glossary.md#pki)).
- You need to **hide the message** -> use encryption (signatures usually leave the message in the clear).
- Data never leaves a trusted boundary and integrity is the only concern -> a plain hash or [CRC](_meta/glossary.md#crc) is cheaper.
...
| Property | Meaning |
|---|---|
| **[EUF-CMA](_meta/glossary.md#euf-cma)** | "Existential Unforgeability under Chosen-Message Attack" — the standard security goal: even after seeing many signatures on chosen messages, an attacker cannot forge a signature on any *new* message. |
...
| Deterministic vs randomized | [ECDSA](_meta/glossary.md#ecdsa) needs fresh randomness per signature; EdDSA and RFC 6979 ECDSA derive the nonce deterministically from the key + message. |
...
- **Not hashing, or using a broken hash.** [MD5](_meta/glossary.md#md5)/SHA-1 collisions let an attacker get a signature on a benign document that is also valid for a malicious one (Flame malware forged a Microsoft cert via an MD5 chosen-prefix collision). Sign a collision-resistant digest.
- **Signature malleability.** ECDSA `(r, s)` and `(r, -s mod n)` are both valid. Bitcoin's original txid used the signature, so malleability changed txids (fixed by the low-`s` rule (BIP-146) and [SegWit](_meta/glossary.md#segwit) (BIP-141)). Enforce canonical/low-`s` form.
...
- **Ignoring domain separation / replay.** The same signed bytes may be valid in another context (different chain, different contract). Include chain ID, nonce, and context tags in the signed payload (cf. [EIP-155](_meta/glossary.md#eip-155), EIP-712).

## Trade-offs

| Axis | Notes |
|---|---|
| Speed | Verification is often the hot path (blockchain nodes verify every tx). EdDSA (Ed25519) is fast to sign *and* verify; [RSA](_meta/glossary.md#rsa) verify is fast but sign is slow; RSA keys are large. |
...
| Quantum | RSA/ECDSA/EdDSA all fall to Shor's algorithm on a large quantum computer. Post-quantum options ([ML-DSA](_meta/glossary.md#ml-dsa)/Dilithium, [SPHINCS+](_meta/glossary.md#sphincs)) exist but have far larger signatures. |
| Aggregation | [BLS](_meta/glossary.md#bls) signatures can be **aggregated** — many signatures over the same or different messages compress to one, and support threshold signing. Used by Ethereum's consensus layer for validator attestations. Trade-off: pairing checks are slower per-signature than Ed25519. |
...
| Scheme | Notes |
|---|---|
| **RSA (PKCS#1 v1.5 / [PSS](_meta/glossary.md#pss))** | Oldest, ubiquitous in TLS/PKI. Prefer **PSS** (randomized, provably secure) over v1.5. Large keys/sigs, slow signing. |
...
| **Schnorr (BIP-340)** | Linear structure enables key/signature aggregation ([MuSig](_meta/glossary.md#musig)) and simpler proofs. Bitcoin Taproot uses Schnorr over secp256k1; non-malleable. |
| **BLS** | Aggregatable + threshold-friendly via pairings. Ethereum consensus, drand. Slower verify. |
| **Threshold / multisig ([FROST](_meta/glossary.md#frost), GG20)** | Split the private key among *n* parties; *t* must cooperate to sign. No single key ever exists in one place. Used in custody/[MPC](_meta/glossary.md#mpc) wallets. |
...
- FIPS 186-5 — Digital Signature Standard ([DSS](_meta/glossary.md#dss)): RSA, ECDSA, EdDSA
