...
- The problem has clear resource entities (users, orders, posts, transactions) with CRUD lifecycle operations
...
- The problem involves read-heavy traffic where cacheability at the CDN or reverse proxy layer is a key scaling lever
...
- Over gRPC: You need browser-native compatibility without a proxy; public developer APIs where SDK generation is not guaranteed; when HTTP/1.1 load balancers are in the path
...
**Do not use when:**
- Real-time bidirectional streaming is required (chat, live feeds) → use WebSockets or SSE
...
- HATEOAS (Hypermedia as the Engine of Application State): responses embed links to valid next actions — rarely implemented in practice but theoretically required for "full REST"
...
- **Pagination omitted on collection endpoints:** Returning unbounded lists (`GET /orders`) will cause timeout and OOM issues at scale — always paginate (cursor-based preferred over offset for large datasets)
...
**Interviewer probe: "How do you scale a REST API to 1M RPS?"**
...
**Idempotency keys:**
For non-idempotent operations (payments, order creation) clients pass a client-generated `Idempotency-Key` header. The server stores the key and result for a TTL (~24 hours) and returns the cached result for duplicate requests. This is essential for safe client retries.
