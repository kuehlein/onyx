...
- A recursive solution is required but recursion depth is prohibitive — the call stack itself is a stack, so you can **simulate DFS/recursion explicitly**.
...
- Over a queue: the problem requires LIFO access, not FIFO. If the order of processing must be reversed from the order of arrival, use a stack.
...
- **Using a stack when the problem needs a queue:** confusing BFS (queue, FIFO, shortest path) with DFS (stack, LIFO). BFS cannot be correctly implemented with a stack.
