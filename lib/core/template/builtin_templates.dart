import 'neutral_template.dart';
import 'software_interviews.dart';
import 'deck_template.dart';
import 'deck_template_yaml.dart';
...
/// once a config-less folder defaults to [neutralTemplate].
const builtInTemplates = <String, DeckTemplate>{
  'software-interviews': softwareInterviewsTemplate,
  'general': neutralTemplate,
};

/// Resolves a discovered config's YAML to a [DeckTemplate]: the built-in template
...
DeckTemplate? resolveDeckTemplate(String yaml) {
  final builtIn = builtInTemplates[templateIdFromYaml(yaml)];
  if (builtIn != null) return builtIn;
  try {
    return deckTemplateFromYaml(yaml);
  } catch (_) {
    return null;
  }
}
